...
document.querySelectorAll(".input-number").forEach(pdctCollection)

function pdctCollection(item)
{
  console.log("pushed");
  inputQuantitiesCollection.push(item);
}

for(i=0;i<inputQuantitiesCollection.length;i++)
{
  document.getElementById("pdct"+(i+1)).addEventListener("change", function()
    {
      console.log("geee");

      if(document.getElementById("pdct"+(i+1)).value > 0)
      {
        console.log(document.getElementById("pdct"+i).value);
      }
    });
